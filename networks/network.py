...
class Network(object):
    ...
    def _conv(self, x, filter_size, out_channel, stride, pad="SAME", trainable=True, name="conv"):
        b, h, w, in_channel = x.get_shape().as_list()
        x = utils._conv(x, filter_size, out_channel, stride, pad, trainable, name)
    ...
    def _deconv(self, x, filter_size, out_channel, stride, pad="SAME", trainable=True, name="conv"):
        b, h, w, in_channel = x.get_shape().as_list()
        x = utils._deconv(x, filter_size, out_channel, stride, pad, trainable, name)
        f = 2 * (h/stride) * (w/stride) * in_channel * out_channel * filter_size * filter_size
        w = in_channel * out_channel * filter_size * filter_size
        scope_name = tf.get_variable_scope().name + "/" + name
        self._add_flops_weights(scope_name, f, w)
    ...
    def _fc(self, x, out_dim, bias=True, trainable=True, name="fc"):
        b, in_dim = x.get_shape().as_list()
        x = utils._fc(x, out_dim, bias, trainable, name)
